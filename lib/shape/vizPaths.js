...
/**
 * Vizra Paths Library
 * A bunch of path drawing functions - each returns a Path2D object, each drawn from the centre
 * @type {Object}
 * @author Ruth John
 * @copyright 2021 Ruth John
 * @namespace
 */
...
const paths = {
	...
	flower: function(size = 100, petals = 4) {
		const controlPoints = petals * 2;
		const radius = size / 2;
		const roundness = 0.666;
		const thickness = 0.5;
		const twist = 0;
		...
		for (let i = 0; i < controlPoints; i++) {
			...
			const controlPointTheta = (moveIn ? i + twist + thickness : i - 1 + twist - thickness) * (Math.PI * 2 / controlPoints);

			// set up our points
			const cpx = radius * roundness * Math.cos(controlPointTheta);
			const cpy = radius * roundness * Math.sin(controlPointTheta);
