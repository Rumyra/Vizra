...
// TODO calling makePath() in the getter causes it to be called ALL THE TIME - this is not great - especially if you want the same shape and not different ones (like if they're generated on draw)
 ...
 */
	 ...
	 */
	// get and set path
	// this isn't right - even set is a bit weird here
	get path() {
		this._path = this.makePath();
		return this._path;
	};
...
/**
 * @description A square with sides that curve in
 */
class VizPinCushion extends VizShape {
	constructor({x = 0, y = 0, w = 100, h = 100, rotate = 0, isAlive = true, fill = '#000000', stroke = '#ffffff', strokeWidth = 1.0 } = {}) {
...
// randomise is a val between 0->1 - where 1 is mental
// ^^ this randomisation only serves to give it a hand drawn effect - which is _really_ nice... but we kinda want a way to randomise points inbetween corners
// name points - should in be a curve or straight line
// how random, number of points on each vertices to stop at, corner is only first 2 sides render, isCurved is true all curves, false all straight and null / anything else is a mixture
class VizEsher extends VizShape {
	constructor({randomise = 0.5, points = 1, corner = false, isCurved = null, x = 0, y = 0, w = 100, h = 100, rotate = 0, isAlive = true, fill = '#000000', stroke = '#ffffff', strokeWidth = 1.0 } = {}) {
	...
	// generate our points we want to loop over
	_genCoords() {
		// corners are always fixed -> clockwise from top right
		const corners = [
			{x: this.w/2, y: -this.h/2,}, // top right
			{x: this.w/2, y: this.h/2,}, // bot right
			{x: -this.w/2, y: this.h/2,}, // bot left
			{x: -this.w/2, y: -this.h/2,} // top left
		];

		// how much do we move & in which direction
		let allCoords = [];
		let jump = this.h/(this.points+1);
		let vertical = true;

		corners.forEach( (c, ci) => {

			// work out direction & jump size
			if (ci===1) {
				jump = -this.w/(this.points+1);
				vertical = false;
			} else if (ci===2) {
				jump = -this.h/(this.points+1);
				vertical = true;
			} else if (ci===3) {
				jump = this.w/(this.points+1);
				vertical = false;
			...
			// we need to add a bit of randomness to the points
			// based on of jump multiplied by randomness property & Math.rand
			let randomPX = ( jump*(this.random*Math.random()) );
		...
		if (this.corner) {
			// length index is 3 corners + points*2
			allCoords.length = 3 + (this.points*2);
		} else {
			// quick & dirty add last corner to end
...
// !STARBURST quick & dirty add first corner to end
			// allCoords.push(corners[0]);

	// draw path
	makePath() {
		const esherPath = new Path2D();

		const coords = this._genCoords();
		
		// loop coords & draw
		coords.forEach( (el, i, all) => {

			if (i===0) {
				// moveTo first
				esherPath.moveTo(el.x, el.y);	
			} else {

				// work out whether to draw a curve
				let curve = utils.chance(0.5) ? true : false;
				if (this.isCurved === true) {
					curve = true;
				} else if (this.isCurved === false) {
					curve = false;
				}
				...
				if (curve) {
					...
					// this needs to be the previous elements jump
					let dx = el.x-(all[i-1].jump/2)
					let dy = el.y-(all[i-1].jump/2)

					if (all[i-1].vertical) {
						if (utils.chance(0.5)) {
							dx = el.x+(all[i-1].jump/2)
						}
					} else {
						if (utils.chance(0.5)) {
							dy = el.y+(all[i-1].jump/2)
						}
					}

					esherPath.quadraticCurveTo(dx, dy, el.x, el.y);

				} else {
					// draw line
					esherPath.lineTo(el.x, el.y);
				}
			}
		})
...
export { VizShape, VizCircle, VizRect, VizCross, VizPolar, VizFlower, VizDiamond, VizSVG, VizDrop, VizTri, VizEsher };
