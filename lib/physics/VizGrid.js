...
class VizGrid {
	 ...
	 */
	_isGenerated() {
		return this._type === 'size' || this._type === 'custom' ? false : true;
	}
 ...
 */
	...
	// int for grid ratio
	get sizeRatio() { return this._sizeRatio; }
	set sizeRatio(val) {
		this._sizeRatio = val;
	}
	...
	_getSpaceVal() {
		...
		if (this._distrib === 'tight') {
			...
			space = 12;
		} else if (this._distrib === 'dense') {
			space = 1.5;
		}

		return space*this._sizeRatio;
	}
	/**
	...
	generateCoords() {
		...
		if (this._type === 'square' || this._type === 'size') {

			this._getSquareCoords();

		} else if (this._type === 'iso' || this._type === 'iso2') {
	...
	_getSquareCoords() {
		...
		for (let y = startY; y < maxY; y += this._ySize) {
			for (let x = startX; x < maxX; x += this._xSize) {
				if (this._yCount === 1) { this._xCount++; }
	...
	_getCustomCoords() {
		...
		this._distrib.forEach((el, i) => {
			this._coords.push(new VizVector(el.x, el.y));
		})
	}
	...
	shiftGrid(topAmount = 10) {
		...
		this.coords.forEach( (el, i) => {
			let randomVecX = Math.floor(utils.randomInt(-topAmount, topAmount)*this._canvas.dpr);
			let randomVecY = Math.floor(utils.randomInt(-topAmount, topAmount)*this._canvas.dpr);

			let newVector = new VizVector(randomVecX, randomVecY);
