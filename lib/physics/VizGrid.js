 ...
 * falloff {float} between 0-1 the spread of distribution. 0 means shapes decline quickly, 1 means shapes decline slowly
...
class VizGrid extends Map {
	...
	get xCount() {
		return this._width/this.origSize;
	}
	...
	// GENERATION =========================
	/**
	 * @description make a two dimensional array of x and y coords based on sizing. Includes padding & offset, but not distribution, which is achieved later. Nesting is based on axis: x is default so rows are first array & cols are second.
	 * @returns {this} populates this.grid
	...
	generateCoords() {
		...
		// if axis are x, first arr i is y
		if (this._axis === 'x') {

			this.grid.forEach((arr, dy) => {
				arr.forEach((coord, dx) => {
					this.set([dx, dy], {x: coord[0], y: coord[1]})
				})
			})
